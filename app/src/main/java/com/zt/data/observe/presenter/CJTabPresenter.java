...
public class CJTabPresenter extends BaseMVPPresenter<CJTabContract.View> implements CJTabContract.Presenter {
    ...
    public void addToday(final Context context) {
        DBThreadHelper.startThreadInPool(new DBThreadHelper.ThreadCallback() {
            ...
            protected Object jobContent() throws Exception {
                int count = LitPalUtils.selectCount(TabCJbean.class,"createDate = ?",date);
                if (count>0){
                    return null;
                }
                InputStream inputStream = context.getResources().openRawResource(R.raw.tab_cj);
    ...
    public void query(final String date) {
        DBThreadHelper.startThreadInPool(new DBThreadHelper.ThreadCallback<List<TabCJbean>>() {

            @Override
            protected List<TabCJbean> jobContent() throws Exception {
                return LitPalUtils.selectWhere(TabCJbean.class,"createdate=?",date);
            }

            @Override
            protected void jobEnd(List<TabCJbean> data) {
    ...
    @Override
    public void save(TabCJbean data,String date) {
        data.save();
        query(date);
    }
    ...
    public void out(final String date) {
        DBThreadHelper.startThreadInPool(new DBThreadHelper.ThreadCallback<Boolean>() {
            ...
            protected Boolean jobContent() throws Exception {
                List<TabCJbean> datas = LitPalUtils.selectWhere( TabCJbean.class,"createDate = ?",date);
                if (datas == null || datas.size() < 0) {
                    return false;
                }
                String[] names = ExcelUtil.colName(TabCJbean.class);
                String dir = ExcelUtil.getWorkDir();
                File dirFile = new File(dir);
                if (!dirFile.exists()) {
                    dirFile.mkdir();
                }
                String fname = "深层沉降原始记录表"+date;
